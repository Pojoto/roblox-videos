...
--5+689*4+sin(5^7+3*6*5/5+8*(5+7)-cos(6+9))-78

--to do list
--chagne for loops to while loops -> isntead of for(int i = 1,), while(int i < length), that way we can manipulate i
--we need to manipulate i to move the index along after we chunk numbers/words out.
...
remote.OnServerEvent:Connect(function(player, t)
	text2 = tostring(t)
...
--	for i = 1, #text, 1 do --loops through the string by char
...
--		if(tonumber(char)) then --check if current char is number, if it is we wanna loop through next cahracteres to see how long the number is
--			print("is a number")
--			local chunk = char --variable to store final chunk of the whole number
...
--			while(tonumber(string.sub(text, j, j))) do --checking if the next char is still a number, if it is keep looping (++ index) until we reach end of entire number
...
--				--end
--			end
--		elseif(string.match(operators, char)) then --since the curerent cahr is not a number, lets check if its an operator
--			print("is an operator")
--			if(#opStack == 0) then
--				table.insert(opStack, char)
--			else
--				if(string.find(operators, correction(opStack[#opStack])) < string.find(operators, char)) then -- if there is another operator in the stack already, check if the new operator has priority or not (ooo), if it doesnt then pop out the last operator and place the new one in. otherwies place new one in normally
...
--		elseif(string.match(letters, string.lower(char))) then --since the current cahr is not an operator or number, we'll check if its a letter (it should be)
...
--				--end
--			end
--		elseif(search(operators, char)) then--string.match(operators, char)) then --since the curerent cahr is not a number, lets check if its an operator
--			print("is an operator")
--			if(#opStack == 0) then
--				table.insert(opStack, char)
--				print(search(operators, opStack[#opStack]))--string.find(operators, correction(opStack[#opStack])))
...
--				if(search(operators, opStack[#opStack]) < search(operators, char)) then--string.find(operators, correction(opStack[#opStack])) < string.find(operators, char)) then -- if there is another operator in the stack already, check if the new operator has priority or not (ooo), if it doesnt then pop out the last operator and place the new one in. otherwies place new one in normally
...
--		elseif(search(letters, string.lower(char))) then --string.find(letters, string.lower(char))) then --since the current cahr is not an operator or number, we'll check if its a letter (it should be)
--			local chunk = char --variable to store final chunk of the whole number
...
--			elseif(item == "csc") then

--			elseif(item == "sec") then

--			elseif(item == "cot") then
